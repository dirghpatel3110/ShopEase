...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    // Step 2: Perform Elasticsearch semantic search with broader matching
    const esResponse = await esClient.search({
      ...
      query: {
        bool: {
          should: [
            ...
            {
              script_score: {
                query: { match_all: {} }, // Include all documents for scoring
                script: {
            ...
            // Broader textual match on description
            {
              multi_match: {
                query,
      ...
      _source: ['id', 'name', 'description', 'price', 'retailer_special_discounts', 'manufacturer_rebates', 'warranty_price', 'category', 'likes', 'availableItems', 'image'],
    });
